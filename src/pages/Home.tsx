...
import { 
  ...
  Smile, Home as HomeIcon, Shield, Clock, Calendar, Target
} from 'lucide-react';
...
export const Home: React.FC<HomeProps> = ({ projects }) => {
  ...
  const processSteps = [
    {
      ...
      deliverables: ["Requirements summary", "Feasibility review", "Zoning guide check", "Budget range discussion", "Project timeline overview"]
    },
    {
      id: 2,
      ...
      deliverables: ["3D elevation drawings", "Floor plan layouts", "Finishes checklist", "Vastu compliance review", "Material selection guide"]
    },
    {
      id: 3,
      ...
      deliverables: ["Detailed quote sheets", "Material grade options", "Milestone budget sheet", "Payment schedule plan", "Contingency allocation"]
    },
    {
      id: 4,
      ...
      deliverables: ["Building permit clearance", "RERA submission verification", "Signed builder agreement", "Environmental clearance", "Municipal NOC filing"]
    },
    {
      id: 5,
      ...
      deliverables: ["Pillar reinforcement cage pour", "Utility conceal piping inspection", "Plaster finish checklist", "Slab casting milestones", "Weekly progress reports"]
    },
    {
      id: 6,
      ...
      deliverables: ["Cylinder test reports", "Plumbing pressure test clear", "Detailed finish validation", "Electrical insulation test", "Punch-list closure"]
    },
    {
      id: 7,
      ...
      deliverables: ["Occupancy certificates", "Keys & digital manuals pack", "Warranty setup documentation", "Final walkthrough sign-off", "Maintenance guide handover"]
    }
  ];
  ...
  // Generic phase principles shown as cards under each step's description
  const phaseHighlights = [
    { icon: <MessageSquare className="w-4 h-4" />, label: "Clear Communication" },
    { icon: <Target className="w-4 h-4" />, label: "Goal Alignment" },
    { icon: <FileText className="w-4 h-4" />, label: "Transparent Approach" },
  ];
        ...
        <section className="space-y-12 bg-white p-8 sm:p-12 rounded-3xl border border-gray-100 shadow-sm">
          ...
          <div className="bg-[#FAFAFA] border border-gray-100 rounded-2xl p-6 sm:p-8 flex flex-col md:flex-row gap-8 items-start">
            <div className="w-full md:w-3/5 space-y-4 text-left">
              ...
              {/* Phase principle cards */}
              <div className="flex flex-wrap gap-3 pt-3">
                {phaseHighlights.map((h, i) => (
                  <div key={i} className="flex items-center gap-2 bg-white border border-gray-150 rounded-xl px-4 py-3 shadow-sm">
                    <span className="text-[#C9A227]">{h.icon}</span>
                    <span className="text-xs font-bold text-[#0F172A]">{h.label}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="w-full md:w-2/5 bg-white border border-gray-150 rounded-xl p-5 text-left">
              ...
              <ul className="space-y-0">
                {activeStepData.deliverables.map((item, index) => (
                  <li key={index} className="flex items-center gap-3 text-xs text-gray-700 font-medium py-2.5 border-b border-gray-100 last:border-b-0">
                    <span className="w-5 h-5 rounded-full bg-[#C9A227] flex items-center justify-center shrink-0">
